...
def plot_siratio(class_avgs, nf):
    figions = plt.figure()
    ax2=figions.add_subplot(111)
    ax2.set_yscale('log')
    shift=0
    if nf==1:
        ind=2
        ...
        ind=4
        ax2.set_ylabel('Secondary FLux / Si Flux @ Surface')

    MetClass_color=iter(['red','green','purple','orange','grey','blue','black'])
    MetClass_shape=iter(['D', '^', 'v', '<', 'o', 's',  '>'])
    for n, avgs in enumerate(class_avgs):
        c=next(MetClass_color)
        mk=next(MetClass_shape)
        ie=avgs[1][0] # Determine which element was used as the incident ion
        if ie=='SW' or 'H' in ie:
            del avgs[ind][0]
        nSi=avgs[1].index("Si")
        ratio_Si = avgs[ind]/avgs[ind][nSi]
        ratio_Si_std=ratio_Si*((avgs[ind+1]/avgs[ind])**2+(avgs[ind+1][nSi]/avgs[ind][nSi])**2)**0.5

        tot_iyld=np.sum(avgs[ind])
        pos=[i+shift for i in range(len(avgs[1]))]
        ax2.plot(pos,ratio_Si,color=c,marker=mk,markersize=10,markeredgewidth=0.0,linestyle='')
        ax2.errorbar(pos,ratio_Si,yerr=ratio_Si_std,color=c,linestyle='')
        if n == len(class_avgs)-1:
            pos=[i+shift/2 for i in range(len(avgs[1]))]
            ax2.xaxis.set(ticks=pos,ticklabels=avgs[1])
            ax2.legend(loc=1,fontsize=10)
        shift+=0.2
    #plt.savefig('/home/mikey/Work/Simulations/images/MarsSiRatio.png',dpi=600, bbox_inches='tight')
    plt.show()
    return

def plot_clavg(class_avgs, nf):
    figions = plt.figure()
    ax=figions.add_subplot(111)
    ax.set_yscale('log')
    shift=0
    if nf==1:
        ind=2
        ...
        ind=4
        ax.set_ylabel('Secondary Ion Surface Flux (ions/cm$^2$/s)')
    elif nf==4:
        ind=2
        ax.set_ylabel('Neutral Surface Flux (atom/cm$^2$/s)')

    MetClass_color=iter(plt.cm.viridis(np.linspace(0,0.9,len(class_avgs))))
    MetClass_shape=iter(['D', '^', 'v', '<', 'o', 's',  '>'])
    for n, avgs in enumerate(class_avgs):
        c=next(MetClass_color)
        mk=next(MetClass_shape)
        ...
        if ie=='SW' or 'H' in ie:
            avgs[1]=avgs[1][1:]
            avgs[ind]=avgs[ind][1:]
            avgs[ind+1]=avgs[ind+1][1:]
        nSi=avgs[1].index("Si")
        ratio_Si = avgs[ind]/avgs[ind][nSi]
        ratio_Si_std=ratio_Si*((avgs[ind+1]/avgs[ind])**2+(avgs[ind+1][nSi]/avgs[ind][nSi])**2)**0.5
        print 'Class Averages: {}'.format(avgs[0])
        for y, elem in enumerate(avgs[1]):
            print '{}, {}$\pm${}: vs. Si = {}$\pm${}'.format(avgs[1][y], avgs[ind][y], avgs[ind+1][y], ratio_Si[y], ratio_Si_std[y])

        tot_iyld=np.sum(avgs[ind])
        print 'Total ion yield = {:.3}'.format(tot_iyld)
        pos=[i+shift for i in range(len(avgs[1]))]
        ax.plot(pos,avgs[ind],label=avgs[0][1],color=c,marker=mk,markersize=10,markeredgewidth=0.0,linestyle='')
        ax.errorbar(pos,avgs[ind],yerr=avgs[ind+1],color=c,linestyle='')
        if n == len(class_avgs)-1:
            pos=[i+shift/2 for i in range(len(avgs[1]))]
            ax.xaxis.set(ticks=pos,ticklabels=avgs[1])
            ax.legend(loc=3,fontsize=12)
            plt.title(avgs[0][0])
            tickpos=np.asarray(pos)
            ax.set_xticks(tickpos+0.5,minor=True)
        ...
        shift+=0.2
    if nf==3:
        plt.savefig('/home/mikey/Work/Simulations/images/{}_Surf_Fluxes.png'.format(avgs[0][0].replace(" ","")), dpi=600, bbox_inches='tight')
    plt.show()
    return
