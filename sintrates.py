...
UH2O_bulk = (0.7)*1.602e-12 # (eV) erg, bulk binding energy, 
UH2O_surf = (0.1)*1.602e-12 # (eV) erg, surface binding energy, Watkins 2011
...
Nmob_surf = 0.8 * WH2O / (2 * UH2O_surf) # number of surface molecules mobilized per ionization
Nmob_bulk = 0.8 * WH2O / (2 * UH2O_bulk)
nj_surf = 15
nj_bulk = 3
...
Deff_mc_surf = Nmob_surf*(nj_surf*alat)**2 *invtau/4
Deff_mc_bulk = Nmob_bulk*(nj_bulk*alat)**2 *invtau/6

Deff_esd_mid = 2*alpha_v*d_vac*d_vac*Nmob_bulk*invtau 
Deff_esd_low = (alpha_v*Nmob_bulk*Dvac*invtau/(pi*nH2O*R_iv))**0.5

Deff_surf = Deff_mc_surf + Deff_esd_mid + Deff_esd_low
Deff_bulk = Deff_mc_bulk + Deff_esd_mid + Deff_esd_low

phi_sput = c_geo*Dz*invtau*nH2O # sputtered flux
